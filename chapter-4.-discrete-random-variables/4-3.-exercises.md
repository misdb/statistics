...
1. Determine whether or not the random variable _X_ is a binomial random variable. If so, give the values of _n_ and _p_. If not, explain why not.
...
3. _X_ is a binomial random variable with parameters _n_ = 12 and _p_ = 0.82. Compute the probability indicated.
   1. P\(11\)P\(11\)
   2. P\(9\)P\(9\)
   3. P\(0\)P\(0\)
   4. P\(13\)P\(13\)
4. _X_ is a binomial random variable with parameters _n_ = 16 and _p_ = 0.74. Compute the probability indicated.
   1. P\(14\)P\(14\)
   2. P\(4\)P\(4\)
   3. P\(0\)P\(0\)
   4. P\(20\)P\(20\)
5. _X_ is a binomial random variable with parameters _n_ = 5, _p_ = 0.5. Use the tables in [Chapter 12 "Appendix"](https://saylordotorg.github.io/text_introductory-statistics/s16-appendix.html) to compute the probability indicated.
   1. _P_\(_X_ ≤ 3\)
   2. _P_\(_X_ ≥ 3\)
   3. P\(3\)P\(3\)
   4. P\(0\)P\(0\)
   5. P\(5\)P\(5\)
6. _X_ is a binomial random variable with parameters _n_ = 5, p=0.3–.p=0.3-. Use the table in [Chapter 12 "Appendix"](https://saylordotorg.github.io/text_introductory-statistics/s16-appendix.html) to compute the probability indicated.
   1. _P_\(_X_ ≤ 2\)
   2. _P_\(_X_ ≥ 2\)
   3. P\(2\)P\(2\)
   4. P\(0\)P\(0\)
   5. P\(5\)P\(5\)
7. _X_ is a binomial random variable with the parameters shown. Use the tables in [Chapter 12 "Appendix"](https://saylordotorg.github.io/text_introductory-statistics/s16-appendix.html) to compute the probability indicated.
...
9. _X_ is a binomial random variable with the parameters shown. Use the special formulas to compute its mean _μ_ and standard deviation _σ_.
   1. _n_ = 8, _p_ = 0.43
   2. _n_ = 47, _p_ = 0.82
   3. _n_ = 1200, _p_ = 0.44
   4. _n_ = 2100, _p_ = 0.62
...
12. _X_ is a binomial random variable with the parameters shown. Compute its mean _μ_ and standard deviation _σ_ in two ways, first using the tables in [Chapter 12 "Appendix"](https://saylordotorg.github.io/text_introductory-statistics/s16-appendix.html) in conjunction with the general formulas μ=Σx P\(x\)μ=Σx P\(x\) and σ=\[Σx2 P\(x\) \]−μ2−−−−−−−−−−−−−√σ=\[Σx2 P\(x\) \]−μ2, then using the special formulas μ=npμ=np and σ=npq−−−√.σ=npq.
    1. _n_ = 10, _p_ = 0.25
    2. _n_ = 15, _p_ = 0.1
13. _X_ is a binomial random variable with parameters _n_ = 10 and p=1∕3.p=1∕3. Use the cumulative probability distribution for _X_ that is given in [Chapter 12 "Appendix"](https://saylordotorg.github.io/text_introductory-statistics/s16-appendix.html) to construct the probability distribution of _X_.
14. _X_ is a binomial random variable with parameters _n_ = 15 and p=1∕2.p=1∕2. Use the cumulative probability distribution for _X_ that is given in [Chapter 12 "Appendix"](https://saylordotorg.github.io/text_introductory-statistics/s16-appendix.html) to construct the probability distribution of _X_.
...
3. An appliance store sells 20 refrigerators each week. Ten percent of all purchasers of a refrigerator buy an extended warranty. Let _X_ denote the number of the next 20 purchasers who do so.
...
6. One-third of all patients who undergo a non-invasive but unpleasant medical test require a sedative. A laboratory performs 20 such tests daily. Let _X_ denote the number of patients on any given day who require a sedative.
   1. Verify that _X_ satisfies the conditions for a binomial random variable, and find _n_ and _p_.

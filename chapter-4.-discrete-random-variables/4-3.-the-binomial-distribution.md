...
The random variable _X_ is binomial with parameters _n_ = 5 and _p_ = 0.17, and q=1−p=0.83.q=1−p=0.83. Thus its mean and standard deviation are

μ=np=5⋅0.17=0.85  \(exactly\)μ=np=5·0.17=0.85  \(exactly\)

and

σ=npq−−−√=5⋅0.17⋅0.83−−−−−−−−−−√=.7055−−−−−√≈0.8399  
...
1. The probability sought is P\(6\).P\(6\). The formula givesP\(6\)=10!\(6!\)\(4!\)\(.5\)6.54=0.205078125P\(6\)=10!\(6!\)\(4!\)\(.5\)6.54=0.205078125

   Using the table,  
   P\(6\)=P\(X≤6\)−P\(X≤5\)=0.8281−0.6230=0.2051P\(6\)=P\(X≤6\)−P\(X≤5\)=0.8281−0.6230=0.2051  

2. The student must guess correctly on at least 60% of the questions, which is 0.60⋅10=60.60·10=6 questions. The probability sought is _not_ P\(6\)P\(6\) \(an easy mistake to make\), but  
   P\(X≥6\)=P\(6\)+P\(7\)+P\(8\)+P\(9\)+P\(10\)P\(X≥6\)=P\(6\)+P\(7\)+P\(8\)+P\(9\)+P\(10\)

  
   Instead of computing each of these five numbers using the formula and adding them we can use the table to obtain  
   P\(X≥6\)=1−P\(X≤5\)=1−0.6230=0.3770P\(X≥6\)=1−P\(X≤5\)=1−0.6230=0.3770
...
Let _X_ denote the number of service calls today on which the part is required. Then _X_ is a binomial random variable with parameters _n_ = 5 and p=1/3=0.3–.p=1∕3=0.3-.

1. Note that the probability in question is not P\(1\)P\(1\), but rather _P_\(_X_ ≤ 1\). Using the cumulative distribution table in [Chapter 12 "Appendix"](https://saylordotorg.github.io/text_introductory-statistics/s16-appendix.html),P\(X≤1\)=0.4609P\(X≤1\)=0.4609
2. The answer is the smallest number _x_ such that the table entry P\(X≤x\)P\(X≤x\) is at least 0.9500. Since P\(X≤2\)=0.7901P\(X≤2\)=0.7901 is less than 0.95, two parts are not enough. Since P\(X≤3\)=0.9547P\(X≤3\)=0.9547 is as large as 0.95, three parts will suffice at least 95% of the time. Thus the minimum needed is three.

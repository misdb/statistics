...
{% tab title="R Source" %}
```text
library(Rstat)# Data x <- matrix(c(35, 12, 5, 6, 24, 18), nrow=2, byrow=TRUE)alpha <- 0.01# Chi-square Statisticct <- chisq.test(x); ct# Graph of the Chi-square Testchitest.plot2(stat=ct$stat, df=ct$para, alp=alpha, side="up", pup=0.99999999 )
```
{% endtab %}

{% tab title="Test Statistic" %}
```text
> ct <- chisq.test(x); ct## ##         Pearson's Chi-squared test#### data:  x## X-squared = 31.751, df = 2, p-value = 1.275e-07##>
```
{% endtab %}
...
```text
> chitest.plot2(stat=ct$stat, df=ct$para, alp=alpha, side="up", pup=0.99999999 )## P-v = 0 
```
{% endtab %}
{% endtabs %}

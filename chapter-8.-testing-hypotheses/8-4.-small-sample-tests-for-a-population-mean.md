...
> * If  $$σ$$  is unknown: $$t =  \frac {(\bar{x}−μ_0)}{(s/\sqrt{n})}$$
>
...
* **Step 3**. From the data we compute $$\bar{x}=169$$ and $$s = 10.39$$ .                Inserting these values into the formula for the test statistic gives              $$T =  \frac {(\bar{x}−μ_0)}{(s/\sqrt{n})} =  \frac {(169−179)}{10.39/\sqrt{5}}  = -2.152$$
* **Step 4.** Since the symbol in _Ha_ is “&lt;” this is a left-tailed test, so there is a single critical value, $$−t_α=−t_{0.05}[df=4]$$ . Reading from the row labeled $$df=4$$ in [Figure 12.3 "Critical Values of "](https://saylordotorg.github.io/text_introductory-statistics/s16-appendix.html) its value is −2.132. The rejection region is $$(−∞,−2.132].$$ 
* **Step 5.** As shown in [Figure 8.12 "Rejection Region and Test Statistic for "](https://saylordotorg.github.io/text_introductory-statistics/s12-04-small-sample-tests-for-a-popul.html#fwk-shafer-ch08_s04_f02) the test statistic falls in the rejection region. **The decision is to reject** $$H_0$$ . 
...
* **Step 3.** From the data we compute $$\bar{x}=0.02075$$ and $$s = 0.00171.$$                Inserting these values into the formula for the test statistic gives               $$T =  \frac {(\bar{x}−μ_0)}{(s/\sqrt{n})} =  \frac {(0.02075−0.02)}{0.00171/\sqrt{4}} = 0.877$$
* **Step 4.** Since the symbol in $$H_a$$ is “ $$≠$$ ” this is a **two-tailed test**,                so there are **two critical values**, $$±t_{α∕2}=±t_{0.005}[df=3].$$                Reading from the row in [Figure 12.3 "Critical Values of "](https://saylordotorg.github.io/text_introductory-statistics/s16-appendix.html) labeled $$df=3$$                their values are $$±5.841.$$                The rejection region is $$(−∞,−5.841]∪[5.841,∞).$$ 
* **Step 5.** As shown in [Figure 8.13 "Rejection Region and Test Statistic for "](https://saylordotorg.github.io/text_introductory-statistics/s12-04-small-sample-tests-for-a-popul.html#fwk-shafer-ch08_s04_f03) **the test statistic does not fall in the rejection region**. The decision is **not to rejec**t $$H_0$$ . 
* In the context of the problem our **conclusion** is:

...
![Z Value that Produces a Known Area 0.0125](../.gitbook/assets/image%20%28167%29.png)

**EXAMPLE 12.** Find the value $$z^*$$ of $$Z $$ as determined by the above figure: the value $$z^*$$ that cuts off a left tail of area 0.0125 in the standard normal distribution. In symbols, find the number $$z^*$$ such that $$P(Z<z^*)=0.0125.$$ 
...
**EXAMPLE 13.** Find the value $$z^*$$of $$Z $$ as determined by  Figure : the value $$z^*$$ that cuts off a right tail of area 0.0250 in the standard normal distribution. In symbols, find the number $$z^*$$ such that $$P(Z>z^*)=0.0250.$$ 
...
**EXAMPLE 14.** Find $$z_{.01}$$ and $$−z_{.01}$$ , the values of $$Z $$ that cut off right and left tails of area 0.01 in the standard normal distribution.
...
**EXAMPLE 15.**  Find  $$x^*$$ such that $$P(X<x*)=0.9332$$ , where $$X$$ is a normal random variable with mean _μ_ = 10 and standard deviation _σ_ = 2.5.
...
{% tab title="x\*" %}
```text
> qnorm(0.9332, mean=10, sd=2.5)[1] 13.75014
```
{% endtab %}
...
**EXAMPLE 16.**  Find $$x^*$$ such that $$P(X>x^*)=0.65$$ , where _X_ is a normal random variable with mean $$μ = 175$$ and standard deviation $$σ = 12$$ .
...
{% tab title="x\*" %}
```text
> qnorm(0.35, mean=175, sd=12)[1] 170.3762
```
{% endtab %}
...
{% tab title="x\*" %}
```text
> qnorm(1-0.05, mean=510, sd=60)[1] 608.6912
```
{% endtab %}
...
{% tab title="R Code" %}
```text
a <- qnorm(0.125, mean=29, sd=2) ; ab <- qnorm(1-0.125, mean=29, sd=2) ; b
```
{% endtab %}

{% tab title="x\*" %}
```text
> a <- qnorm(0.125, mean=29, sd=2) ; a[1] 26.6993> b <- qnorm(1-0.125, mean=29, sd=2) ; b[1] 31.3007
```
{% endtab %}

{% tab title="Plot" %}
```text
pnormGC(c(26.6993, 31.3007), region="between", mean=29, sd=2, graph=TRUE)abline(h=0, col="gray")
```
...
**Ex 2\)** Find  $$x^*$$ such that $$P(X<x*)=0.9332$$ , where $$X$$ is a normal random variable with mean $$μ = 10$$ and standard deviation $$σ = 2.5$$ .

{% tabs %}
{% tab title="R Code" %}
```text
x <- qnorm(0.9332, mean=10, sd=2.5); x <- round(x, 2); xpnormGC(x, region = "below", mean=10, sd=2.5, graph=TRUE)
```
{% endtab %}

{% tab title="x\*" %}
```text
> x <- qnorm(0.9332, mean=10, sd=2.5); x <- round(x, 2); x[1] 13.75
```
{% endtab %}

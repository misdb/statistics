...
In actual practice _p_ is not known, hence neither is $$σ_\hat{P}$$. In that case in order to check that the sample is sufficiently large we substitute the known quantity $$\hat{p}$$ for $$p$$ . This means checking that the interval
...
1.    $$μ_\hat{P}=p$$  $$ σ_\hat{P}= \sqrt{ \frac{pq}{n}}$$ 
2. $$P(0.33<\hat{P}<0.43)$$ 

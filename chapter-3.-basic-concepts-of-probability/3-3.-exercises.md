...
**Ex 14.** Suppose for events _A_ and _B_ connected to some random experiment, $$P(A)=0.50$$ and $$P(B)=0.50$$ . Compute the indicated probability, or explain why there is not enough information to do so.
...
**Ex 17.** The sample space that describes all three-child families according to the genders of the children with respect to birth order is  
...
**Ex 22.** The sample space of equally likely outcomes for the experiment of rolling two fair dice is  
...
**Ex 30.** An economist wishes to ascertain the proportion _p_ of the population of individual taxpayers who have purposely submitted fraudulent information on an income tax return. To truly guarantee anonymity of the taxpayers in a random survey, taxpayers questioned are given the following instructions.
...
3. Equate the sum of the entries in the three cells in the table in \(a\) that together correspond to the answer “Yes” to the number _r_ to obtain the formula $$p=2r−1$$ that expresses the unknown number _p_ in terms of the known number $$r$$ .
4. Use the fact that $$r+s=1$$ \(since they are the probabilities of complementary events\) to verify that the formulas in \(b\) and \(c\) give the same value for $$p$$ . \(For example, insert $$s=1−r$$ into the formula in \(b\) to obtain the formula in \(c\).\)

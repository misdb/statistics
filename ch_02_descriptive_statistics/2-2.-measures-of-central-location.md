...
**Example 11.** 다음 데이터 세트의 sample median을 구하라.
...
**Example 12.** 다음 데이터 세트의 mode\(최빈값\)를 구하라.
...
**Example 13.** MASS 라이브러리의 Cars93 데이터 세트를 이용하여  차종별 가격\(Price by Type\)의 분포를 히스토그램으로 그리고, 차종별 가격의 skewness\(왜도, 좌우 대칭 정도\)와 kurtosis\(첨도, 정규분포 대비 봉우리 높이 정도\)를 구하라.

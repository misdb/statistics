...
**Example 1\)** 다음 데이터 세트의 $$\Sigma x$$ , $$\Sigma x^2$$ , $$\Sigma {(x-1)^2}$$ 을 구하시오.
...
**Example 4\)** 가임기 년령을 넘긴 무작위로 선발된 19명의 여성의 데이터가 다음과 같다\( $$x$$ 는 자녀의 수, $$f$$ 는 그 값의 빈도\). 표본의 평균을 구하라.
...
**Example 5.** 첫번째 해에는 5%, 두번째 해에는 10% 증가했다. 연평균 증가율은?
...
**Example 6.** 갈 때 10m/s, 올때 20m/s 로 주행하였다. 평균 속력은?
...
**Example 7.** 기업 직원의 연 평균 수입에 관심이 있다고 가정하자. 무작위로 선발된 직원 7명의 대략적인 연 수입은 다음과 같다\(단위 : 천 달러\)
...
**Example 8.** 다음 데이터 세트의 sample median을 구하라.
...
**Example 9.** 다음 데이터 세트의 sample median을 구하라.
...
**Example 10.** 다음 데이터 세트의 sample median을 구하라.
...
### 2.2 Skewness and Kurtosis in R
...
**Example 11.** MASS 라이브러리의 Cars93 데이터 세트를 이용하여  차종별 가격\(Price by Type\)의 분포를 히스토그램으로 그리고, 차종별 가격의 skewness\(왜도, 좌우 대칭 정도\)와 kurtosis\(첨도, 정규분포 대비 봉우리 높이 정도\)를 구하라.
...
**Example 11.** 다음 데이터 세트의 mode\(최빈값\)를 구하라.
